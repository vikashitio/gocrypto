...
func Loginhistory(c *fiber.Ctx) error {
	...
	database.DB.Db.Table("login_history").Order("token_id desc").Limit(limit).Offset(offset).Where("client_id = ?", LoginMerchantID).Find(&loginHistory)

	var total int64
	database.DB.Db.Table("login_history").Count(&total)
